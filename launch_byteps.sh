...
worker_docker=haibinlin/py3-gluon-worker:1
...
do
  host=${host%% slots*}
  echo "nvidia-docker run --security-opt seccomp=seccomp.json -d -v ~/.ssh:/root/.ssh --network=host --shm-size=32768m $worker_docker bash -c 'cd gluon-nlp/scripts/bert/; bash bps_worker.sh $num_server $num_worker $root_ip $root_port worker $count $net $batch_size $byte_part_size $byte_num_rings $byte_num_groups $byte_pcie $byte_load_balance $async' on $host"
  ssh -o "StrictHostKeyChecking no" $host tmux new-session -d "nvidia-docker run --security-opt seccomp=seccomp.json -d -v ~/.ssh:/root/.ssh --network=host --shm-size=32768m $worker_docker bash -c 'cd gluon-nlp/scripts/bert/; bash bps_worker.sh $num_server $num_worker $root_ip $root_port worker $count $net $batch_size $byte_part_size $byte_num_rings $byte_num_groups $byte_pcie $byte_load_balance $async'"
  let "count+=1"
