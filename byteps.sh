if [ $# -le 3 ]; then
    echo 'usage: ';
    echo 'bash byteps.sh num_gpus server worker ip        port role       id model bs';
    echo 'bash byteps.sh 8        1      1      127.0.0.1 1234 scheduler';
    echo 'bash byteps.sh 8        1      1      127.0.0.1 1234 worker     0  bert_24_1024_16 12';
...
if [ $DMLC_ROLE = 'server' ]; then
  ...
  python3 /usr/local/byteps/launcher/launch.py \
       ...
       --accumulate 1 --model $MODEL --batch_size_eval $BS --backend byteps
